...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const examStatsResponse = await fetch(`http://localhost:8001/exam-attempts/user/${user.id}/stats`)
      if (examStatsResponse.ok) {
        ...
        const examAttemptsResponse = await fetch(`http://localhost:8001/exam-attempts/user/${user.id}`)
      ...
      const vocabStatsResponse = await fetch(`http://localhost:8001/user-vocabularies-progress/user/${user.id}/stats`)
      ...
      const grammarStatsResponse = await fetch(`http://localhost:8001/user-grammar-progress/user/${user.id}/stats`)
      ...
      const recentExamsResponse = await fetch(`http://localhost:8001/exam-attempts/user/${user.id}`)
  ...
  const calculateLearningStreak = async (userId: string | number): Promise<number> => {
    try {
      ...
      const response = await fetch(`http://localhost:8001/exam-attempts/user/${userId}`)
      if (response.ok) {
